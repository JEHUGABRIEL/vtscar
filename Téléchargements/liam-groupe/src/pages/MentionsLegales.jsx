      ...
      <section className="pt-[140px] pb-24 px-6" ref={sectionRef}>
        <div className="max-w-3xl mx-auto">
          <div className="reveal">
            <SectionHeading
              icon={ScrollText}
              eyebrow={t("legal.mentions.eyebrow")}
            ...
            {list.map((s, i) => (
              <div key={i} className="reveal">
                ...
                <p className="text-gray-500 leading-relaxed">
                  {s.text?.replace("{{name}}", siteInfo.fullName || "LIAM Groupe").replace("{{email}}", siteInfo.emails?.[0] || "").replace("{{address}}", (siteInfo.address || []).join(", "))}
                </p>
              </div>
            ))}
          </div>
