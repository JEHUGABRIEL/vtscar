...
export default function EventsSection() {
  ...
  return (
    <div className="mt-8">
      ...
      <AdminFormModal open={showForm} onClose={() => setShowForm(false)} title={edit ? "Modifier l'\u00e9v\u00e9nement" : 'Nouvel \u00e9v\u00e9nement'}>
        <div className="space-y-3">
          <div className="grid gap-3 sm:grid-cols-2">
            <input value={form.id} onChange={(e) => setForm({ ...form, id: e.target.value })} placeholder="ID (ex: mon-event)" className="rounded-lg border border-white/15 bg-zinc-800/80 px-3 py-2.5 text-xs text-white outline-none placeholder:text-white/30" disabled={!!edit} />
            <input value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })} placeholder="Slug (ex: mon-event)" className="rounded-lg border border-white/15 bg-zinc-800/80 px-3 py-2.5 text-xs text-white outline-none placeholder:text-white/30" />
            <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} placeholder="Titre de l'\u00e9v\u00e9nement" className="rounded-lg border border-white/15 bg-zinc-800/80 px-3 py-2.5 text-xs text-white outline-none placeholder:text-white/30" />
            <input value={form.eventDate} onChange={(e) => setForm({ ...form, eventDate: e.target.value })} type="date" className="rounded-lg border border-white/15 bg-zinc-800/80 px-3 py-2.5 text-xs text-white outline-none" />
            <input value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} placeholder="Lieu" className="rounded-lg border border-white/15 bg-zinc-800/80 px-3 py-2.5 text-xs text-white outline-none placeholder:text-white/30" />
            <div className="sm:col-span-2">
              <ImageUpload value={form.image} onChange={(v) => setForm({ ...form, image: v })} label="Image de l'\u00e9v\u00e9nement" />
            </div>
            <textarea value={form.excerpt} onChange={(e) => setForm({ ...form, excerpt: e.target.value })} placeholder="Extrait (court)" rows={2} className="rounded-lg border border-white/15 bg-zinc-800/80 px-3 py-2.5 text-xs text-white outline-none placeholder:text-white/30 sm:col-span-2" />
            <textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Description compl\u00e8te" rows={3} className="w-full rounded-lg border border-white/15 bg-zinc-800/80 px-3 py-2.5 text-xs text-white outline-none placeholder:text-white/30 sm:col-span-2" />
          </div>
